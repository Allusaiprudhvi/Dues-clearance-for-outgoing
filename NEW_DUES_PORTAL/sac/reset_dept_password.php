...
{  
    ...
    $s1="select * from department where dp_name='$dp_name'";   
    
    $res1=$conn->query($s1);
    if($res1->num_rows!=0){
            $new_password=sha1(sha1(strtolower($dp_name)).sha1("mySalt@$#(%"));
        $s2="update department set password='$new_password' where dp_name='$dp_name'";
                 $res3=$conn->query($s2);
                 if($res3==false)
    ...
    {   
        echo "<script>alert('Password Updated Successfully');</script>";
                ...
                $message="Department Id doesn't exist";
		...
		<div class="content">
			<div class="top-grids">
					...
					<div class="signin-form reset-password">
						<h3>Reset Department Password</h3>
			                         <form action="" method="post" >	
				                            <p style="float:red;color: red"><?php echo $message;?></h3>
									<input  type="text" placeholder="Enter Department id" name="dp_name"  required="">
