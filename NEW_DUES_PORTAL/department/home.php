...
$sql="select * from department where dp_name='$dp_name'";
$res=$conn->query($sql);
$row=$res->fetch_array();
...
if (isset( $_POST['import']))
{
    ...
    $_SESSION['dis']=1;
       $s4="select * from department where dp_name='$dp_name'";
       $res4=$conn->query($s4);
       if($res4==false)
         die($conn->error);
        ...
        for($i=0;$i<$sheetCount;$i++)
        {
            ...
            foreach ($Reader as $Row)
            {
              ...
              if ((!empty($roll_no) || !empty($dues))&&($roll_no!="Roll. No."))
                    ...
                    $s1="select * from stu_dpt_dues where roll_no='$roll_no' and dp_name='$dp_name'";
                    $res=$conn->query($s1);
                    ...
                    {
                        $approve=0;
                        $query = "insert into stu_dpt_dues(roll_no,dp_name,dues,approve) values('$roll_no','$dp_name','$dues','$approve')";
                    ...
                    else
                    {
                      ...
                      while($row=$res->fetch_array())
                        ...
                        $dues=$dues+$pre_dues;                           
                        $query = "update stu_dpt_dues set dues='$dues',approve='$approve' where roll_no='$roll_no' and dp_name='$dp_name'";
                        $result = $conn->query($query);
          ...
          if (! empty($result)){
                        ...
                        $v=true;
                        $query = "update department set due_updated='$date',uploaded='$v' where dp_name='$dp_name'";
                        $result = $conn->query($query);
                        if($result==false)
                        die($conn->error);
                        $type = "success";
                        $message = "Students list Uploaded Successfully".$targetPath;
                        } 
                        else {
                        $type = "error";
...
<html lang="en">
<head>
    <title><?php echo $department;?> Department</title>
...
<body>
  ...
  <section id="upload_dues" > 
		...
		<div class="content">
			<div class="top-grids">
			         ...
			         <div class="signin-form subscribe">	
						<h3 style="font-weight: 600;" >Upload Dues List</h3>    
                         ...
                         <a  href="dept_form.xlsx" class="buttonDownload" download="DuesForm">Download Student Form</a>
   ...
   <script type="text/javascript">
       ...
       /* function showORhide(){
        if(document.getElementByClassName('upload_dues').display!=)
        }
*/   </script> 
